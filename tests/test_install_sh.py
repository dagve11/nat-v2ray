...
class InstallScriptTests(unittest.TestCase):
    ...
    def test_http_and_socks5_proxy_options_render_xray_configs(self) -> None:
        ...
        self.assertIn("plain_proxy_install_common()", script)
        self.assertIn("'http-proxy'", script)
        self.assertIn("'socks5-proxy'", script)
        self.assertIn("'HTTP_USER'", script)
        self.assertIn("'HTTP_PASS'", script)
        self.assertIn("'SOCKS_USER'", script)
        self.assertIn("'SOCKS_PASS'", script)
        self.assertIn("PROTOCOL=${protocol}", script)
        self.assertIn("${user_key}=${user}", script)
        self.assertIn("${pass_key}=${pass}", script)
        self.assertIn("输入 - 表示无认证", script)
        self.assertIn('if [ "${user}" = "-" ]; then', script)
