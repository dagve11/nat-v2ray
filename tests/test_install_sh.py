...
class InstallScriptTests(unittest.TestCase):
    ...
    def test_interactive_reads_do_not_force_extra_blank_lines(self) -> None:
        ...
        self.assertIn('IFS= read -r -e "${target}"', read_body)
    ...
    def test_acme_install_skips_crontab_precheck_on_minimal_systems(self) -> None:
        ...
        self.assertIn('local installer="/tmp/nat-v2ray-get-acme.sh"', install_body)
        self.assertIn('curl -fsSL https://get.acme.sh -o "${installer}"', install_body)
        self.assertIn('sh "${installer}" --force', install_body)
