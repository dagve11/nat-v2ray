...
VLESS Reality、VLESS TCP、VLESS WS、VLESS HTTPUpgrade、VLESS TCP TLS、VLESS WS TLS、VLESS gRPC TLS、Trojan TLS、Trojan WS TLS、Trojan gRPC TLS、VMess TCP、VMess WS、VMess HTTPUpgrade、VMess TCP TLS、VMess WS TLS、VMess gRPC TLS 使用 TCP。NAT 面板必须把外部端口的 `TCP` 转发到本机端口。
...
VLESS TCP / VLESS WS / VLESS HTTPUpgrade / VMess TCP / VMess WS / VMess HTTPUpgrade 默认不带 TLS，适合临时测试或兼容旧客户端；公网长期使用建议优先 HY2、Reality 或 TLS 类协议。
