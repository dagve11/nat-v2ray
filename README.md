...
HY2 和 VMess mKCP 使用 UDP。NAT 面板必须把外部端口的 `UDP` 转发到本机端口。

只转发 `TCP` 时，脚本会安装成功，服务也会监听成功，但客户端连接会超时。

VMess mKCP dynamic port 需要在 NAT 面板转发整个 `UDP` 端口范围，例如 `20000-20010`。

## TCP 协议注意事项

VLESS Reality、VLESS WS TLS、VLESS gRPC TLS、Trojan TLS、Trojan WS TLS、Trojan gRPC TLS、VMess TCP、VMess WS、VMess WS TLS、VMess gRPC TLS 使用 TCP。NAT 面板必须把外部端口的 `TCP` 转发到本机端口。

Reality 不需要申请证书，适合没有域名控制权或不想做 DNS TXT 的场景。

VLESS/VMess/Trojan 的 TLS 类协议会使用 DNS-01 手动 TXT 申请证书，不依赖 80/443 端口。

VMess TCP / VMess WS 默认不带 TLS，适合临时测试或兼容旧客户端；公网长期使用建议优先 HY2、Reality 或 TLS 类协议。
