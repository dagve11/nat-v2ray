...
VLESS Reality、VLESS WS TLS、Trojan TLS、VMess TCP、VMess WS 使用 TCP。NAT 面板必须把外部端口的 `TCP` 转发到本机端口。

Reality 不需要申请证书，适合没有域名控制权或不想做 DNS TXT 的场景。

VLESS WS TLS 和 Trojan TLS 会使用 DNS-01 手动 TXT 申请证书，不依赖 80/443 端口。
