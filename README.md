...
`nv uninstall` 提供两种方式：**保留配置，只卸脚本**（节点照常使用，仅移除 `nv` 命令）或**完全卸载**（停止服务，并删除脚本安装的二进制、systemd 服务、配置目录、证书目录、日志目录、geo 数据和 `nv` 命令）。
...
`2) 更改配置` 原地编辑现有节点，重新走该协议安装流程，所有字段默认显示当前值，回车即保留。改 host、端口或协议参数后仍覆盖原 profile，profile 名和数量不变。

- TLS 域名不变时复用已有证书，不重新申请、不触发 TXT；域名改变才走 DNS-01 流程。
- Reality 编辑复用原私钥/公钥/shortId，不重新生成，分享链接不变。
